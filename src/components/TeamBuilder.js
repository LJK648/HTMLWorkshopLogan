...
const TeamBuilder = () => {
    ...
    const fetchTeams = async () => {
        try {
            ...
            if (response.ok) setAllTeams(await response.json());
        } catch (error) {
    ...
    const getPlayerPositionForSport = (player, sport) => {
        ...
        return sp[sport] || player.position || 'N/A';
    };
    ...
    const addPlayerToRoster = (player) => {
        ...
        if (roster.some(r => r.email === playerEmail)) { showMessage('Player already in roster.', 'warning'); return; }
        if (teamCount >= maxTeams) { showMessage(`Player has reached max team limit (${maxTeams}).`, 'warning'); return; }

        const playerSports = getPlayerSports(player);
        const rosterSport = getRosterSport();

        if (rosterSport && rosterSport !== 'all') {
            if (!playerSports.includes(rosterSport)) { showMessage('Player not registered for roster sport.', 'warning'); return; }
        }

        if (currentTeamSport !== 'all' && rosterSport === null) {
            if (!playerSports.includes(currentTeamSport)) { showMessage('Player not registered for selected sport.', 'warning'); return; }
    ...
    const removePlayerFromRoster = (email) => setRoster(roster.filter(r => r.email !== email));

    const createTeam = async () => {
        if (!teamName.trim()) { showMessage('Please enter a team name.', 'danger'); return; }
        const rosterSport = getRosterSport();
        if (!rosterSport || roster.length === 0) { showMessage('Select at least one player for the team.', 'danger'); return; }
            ...
            const response = await fetch(`${API_URL}/api/teams`, {
                ...
                body: JSON.stringify({
                    ...
                    players: roster.map(p => ({
                        ...
                        sport: rosterSport, position: getPlayerPositionForSport(p, rosterSport)
                    }))
                })
            });
    ...
    const renderPlayerPool = () => {
        ...
        const filtered = allPlayers.filter(player => {
            const sports = getPlayerSports(player);
            const matchSearch = !search || player.fullName.toLowerCase().includes(search) || sports.join(' ').toLowerCase().includes(search) || player.email.toLowerCase().includes(search);
            const matchSport = selectedSport === 'all' || sports.includes(selectedSport);
            return matchSearch && matchSport;
        });

        if (filtered.length === 0) return <div className="alert alert-secondary">No players match your search and/or sport filter.</div>;
        ...
        return filtered.map(player => {
            ...
            if (!disabled && rosterSport && rosterSport !== 'all') {
                if (!playerSports.includes(rosterSport)) { disabled = true; reasonText = ' (Not registered for roster sport)'; }
            }

            return (
                <div key={playerEmail} className="mb-2 d-flex justify-content-between align-items-center">
                    <span>{player.fullName} ({playerSports.join(', ')}) {reasonText}</span>
                    <button className="btn btn-sm btn-primary" onClick={() => addPlayerToRoster(player)} disabled={disabled}>Add</button>
                </div>
            );
        });
    };
                ...
                <div className="row">
                    <div className="col-lg-6">
                        ...
                        <div className="mb-3">
                            ...
                            <input type="text" className="form-control" placeholder="Enter team name" value={teamName} onChange={(e) => setTeamName(e.target.value)} />
                        </div>
                            ...
                            {roster.length === 0 ? (
                                ...
                                roster.map((player, idx) => (
                                    <div key={idx} className="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        <button className="btn btn-sm btn-danger" onClick={() => removePlayerFromRoster(player.email)}>Remove</button>
                                    </div>
                                ))
                            )}
                        </div>
                        <button className="btn btn-success" onClick={createTeam} disabled={loading}>
                    ...
                    <div className="col-lg-6">
                        ...
                        <div className="mb-3">
                            ...
                            <select className="form-select" value={teamSport} onChange={(e) => setTeamSport(e.target.value)}>
                        ...
                        <div id="teamBuilderList">{renderPlayerPool()}</div>
                    </div>
                </div>
            </main>
        </div>
